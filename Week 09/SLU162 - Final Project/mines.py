...
class MinesweeperGame:
    ...
    def check_if_mine(self, x: int, y: int) -> bool:
        ...
        True
        """
        # TODO: check if mine is in (x,y) position
        #for shot in self.mines:
        #    if self.mines[0] == x and self.mines[1] == y:
        #        return True
        #return False 
    ...
    def count_mines_around(self, x: int, y: int) -> int:
        ...
        1
        """
        # TODO: counts the mines around (x,y) position
        
        minas = 0
        bolta = [(x-1, y-1), (x, y-1), (x+1, y-1), (x-1, y),  (x+1, y), (x-1, y+1), (x, y+1), (x+1, y+1)]

        for bolta_x, bolta_y in bolta:
            if bolta_x >= 0 and bolta_x < len(self.mines) and bolta_y >= 0 and bolta_y < len(self.mines[0]):
                if self.check_if_mine(bolta_x, bolta_y):
                    minas += 1
